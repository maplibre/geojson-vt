...
import type { GeoJSONVTFeature, StartEndSizeArray } from './feature';
...
export function convert(data: GeoJSON.GeoJSON, options: GeoJSONVTOptions): GeoJSONVTFeature[] {
    ...
    switch (data.type) {
        ...
        case 'Feature':
            ...
            break;
        default:
            convertFeature(features, {geometry: data, type: "Feature", properties: {}}, options);
    }
...
function convertFeature(features: GeoJSONVTFeature[], geojson: GeoJSON.Feature, options: GeoJSONVTOptions, index?: number) {
    ...
    if (!geojson.geometry?.coordinates || geojson.geometry.coordinates.length === 0) return;
    ...
    switch (geojson.geometry.type) {
        case 'Point': {
            ...
            return;
        case 'MultiPoint':{
                const multiPointGeometry: number[] = [];
        ...
        case 'MultiLineString':
            if (options.lineMetrics) {
                ...
                for (const line of geojson.geometry.coordinates) {
                    let lineGeometry: StartEndSizeArray = [];
                    convertLine(line, lineGeometry as StartEndSizeArray, tolerance, false);
